...
compteur_afficher_city = 0

@router.get('/')
async def afficher_city():
    """
    Récupère et affiche les données de la table 'city'.

    Retours :
        dict : Un dictionnaire contenant les données de la table 'city'.

    Erreurs HTTP possibles :
        - HTTP 500: En cas d'erreur de base de données.
        - HTTP 404: Si aucune donnée n'est trouvée dans la table 'city'.
    ...
    Exemple :
        Pour récupérer les données de la table 'city', envoyez une requête GET à cette URL.
    """
    try:
